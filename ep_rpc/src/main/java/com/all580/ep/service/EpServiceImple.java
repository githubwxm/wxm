...
public class EpServiceImple implements EpService {
    ...
    public Result<CoreEpAccess> createPlatform(Map map) {
        map.put("status",0);// 状态默认未初始化
        map.put("status_bak",0);// 状态默认未初始化
        map.put("ep_type",12);
    ...
    public Result<PlatformEp> validate(Map params) {
        ...
        if(null!=access){
            ...
            try {
                ...
                platformEp.setCapital(null);//TODO   t_capital(查询通道费率)
    ...
    public Result<Integer> getEpStatus(Integer id){
        ...
        while(true){
            ...
            Ep ep = list.get(0);
             ...
             status = ep.getStatus();
            if(status!=1||ep.getEp_type()==12) {//查到平台商 或者不是正常的就直接返回
                result.put(status);
                break;
            }
        }
