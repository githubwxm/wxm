...
public class ProductController extends BaseController {
    ...
    @ResponseBody
    public Result<?> selectEpSelfProduct(@RequestParam("statistic_ep_id") Integer ep_id,String pro_name,Integer type,
                                         Integer record_start,Integer record_coun){
        if(null==type){
            type=ProductConstants.ProductType.SCENERY;
        }
        return productService.selectEpSelfProduct(ep_id,pro_name,type,record_start,record_coun);
    }
    /**
     * 查询景区子产品
     * @param    list
     * @return
     */
    @RequestMapping(value = "select_ids", method = RequestMethod.GET)
    @ResponseBody
    public Result<?> selectIds(Integer [] list,@RequestParam("ep_id") Integer ep_id,Integer record_start,Integer record_count){
        return productService.selectIds(list==null?null:  Arrays.asList(list), ep_id,record_start,record_count);
    }
    /**
     * 修改景区子主产品
     * @param  map  主产品id
    ...
    public Result<?> selectProductSubBatch(@RequestParam("product_sub_id") Integer productSubId,
                                           @RequestParam("ep_id") Integer epId, @RequestParam("status") Integer status){
    ...
    public Result<?> addSceneryProduct(@RequestBody Map params) {
        ...
        switch (Integer.valueOf(params.get("type").toString())) {
            case ProductConstants.ProductType.SCENERY:
                ...
                productService.addSceneryProduct(CommonUtil.objectParseString(params.get("ma_pro_id")),CommonUtil.objectParseString(params.get("ma_name")),CommonUtil.objectParseInteger(params.get("ma_conf_id")),params.get("name").toString(),
